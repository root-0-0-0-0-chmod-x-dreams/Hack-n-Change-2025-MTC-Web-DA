...
> [!NOTE] Для работы схемы сначала необходимо запустить бекэнд, поэтому перед запуском схемы не забудьте прочитать (Интрегральная-схема-1/server/DEPLOY.md). 
...
```
> [!NOTE] В данном случае https://example.ru/stats это заглушка для домена вашего сервера, в реальном использовании замените на свой домен. Также не забудьте указать нужные id таблиц MWS Tables
...
```
Пример ответа есть в Интрегральная-схема-1/Примеры-запросов-ответов/source_table_scheme.json
...
```
Интрегральная-схема-1/Примеры-запросов-ответов/yt-requests-task.md
