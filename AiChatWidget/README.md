...
- Откройте вкладку дополнительно > API > Получить записи и скопируйте из curl ссылки id который идет после ```datasheets/``` 
- Замените в ```index.tsx``` YOUR_TABLE_SHEETID на полученный токен

**Настройка view ID**
- Откройте вкладку дополнительно > API > Получить записи и скопируйте из curl ссылки id который идет после ```viewId/``` 
- Замените в ```index.tsx``` YOUR_VIEWID на полученный токен
